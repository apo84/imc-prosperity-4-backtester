...
def create_backtest_data(
    ...
    if missing_limits:
        missing_limits_str = ", ".join(missing_limits)
        raise ValueError(
            f"Missing position limits for product(s): {missing_limits_str}. "
            "Add these symbols to prosperity4bt.data.LIMITS before running the backtest."
        )
